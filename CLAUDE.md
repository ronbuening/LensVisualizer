...
- **Pupil aberration analysis** — `src/optics/pupilAberration.ts` provides two pure functions: `computePupilAberrationProfile` (entrance pupil — how the apparent EP z-position shifts with field angle, measured via the ratio of solved to paraxial chief-ray launch height) and `computeExitPupilAberrationProfile` (exit pupil — real XP z-position per field angle from full-system chief-ray back-projection: `xpZ = −y'_last / u'_last`). Both accept an optional pre-computed `FieldGeometryState` to avoid redundant traces when called together. No UI tab exists yet — data layer only; see `agent_docs/records/pupil-aberration-analysis.md` for the implementation roadmap.
