...
4. **Cross-gap overlap**: For each air gap, the combined sag intrusion from the preceding element's rear surface (curving forward) and the following element's front surface (curving backward) must not exceed the gap thickness. Otherwise elements overlap in the rendered diagram.
