...
import { computeLocaBarOffsets, REFERENCE_LOCA_MM } from "../../../src/optics/chromaticRayFanScaling.js";

describe("computeLocaBarOffsets", () => {
  const SC = 100; // 100 px/mm

  it("produces proportionally wider offsets for larger LoCA", () => {
    const small = computeLocaBarOffsets({ R: 50.01, G: 50.0, B: 49.99 }, 50.0, 66, SC);
    const large = computeLocaBarOffsets({ R: 50.1, G: 50.0, B: 49.9 }, 50.0, 66, SC);
  ...
  it("centers the G-line reference at zero offset", () => {
    const result = computeLocaBarOffsets({ R: 50.05, G: 50.0, B: 49.95 }, 50.0, 66, SC);
    expect(result.barOffsets.G).toBeCloseTo(0, 10);
  });

  it("clamps bars that exceed the half-width", () => {
    // REFERENCE_LOCA_MM = 0.15, so an offset of 1.0 mm is way beyond reference
    const result = computeLocaBarOffsets({ R: 51.0, G: 50.0, B: 49.0 }, 50.0, 66, SC);
  ...
  it("does not clamp moderate LoCA within reference range", () => {
  ...
  it("handles two-channel case (R+G only)", () => {
    const result = computeLocaBarOffsets({ R: 50.05, G: 50.0 }, 50.0, 66, SC);
  ...
  it("includes violet offsets when present", () => {
    const result = computeLocaBarOffsets({ G: 50.0, V: 49.95 }, 50.0, 66, SC);
    expect(result.barOffsets.G).toBeCloseTo(0, 10);
  ...
  it("scales proportionally with wider view width", () => {
    const narrow = computeLocaBarOffsets({ R: 50.05, G: 50.0, B: 49.95 }, 50.0, 66, SC);
    const wide = computeLocaBarOffsets({ R: 50.05, G: 50.0, B: 49.95 }, 50.0, 132, SC);
    // Double the view width -> double the pixel offsets (same LoCA, unclamped)
  ...
  it("returns zero offsets when all channels have the same intercept", () => {
    const result = computeLocaBarOffsets({ R: 50.0, G: 50.0, B: 50.0 }, 50.0, 66, SC);
  ...
  it("uses REFERENCE_LOCA_MM to set base magnification", () => {
    // A channel exactly at REFERENCE_LOCA_MM offset should fill ~70% of the half-width
    const viewW = 66;
    const result = computeLocaBarOffsets({ R: 50.0 + REFERENCE_LOCA_MM, G: 50.0 }, 50.0, viewW, SC);
