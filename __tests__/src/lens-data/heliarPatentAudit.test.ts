...
import data from "../../../src/lens-data/voigtlander/VoigtlanderHeliar.data.js";
...
describe("Heliar: US716035 inferred finite focus", () => {
  it("images the labeled close object distance in the inferred paraxial unit model", () => {
    ...
    for (const s of data.surfaces) {
      ...
      index = s.nd;
      const gap = s.label === "8" ? data.var["8"][1] : s.d;
  ...
  it("moves every optical surface and stop equally toward the object", () => {
    ...
    const extension = data.var["8"][1] - data.var["8"][0];
