...
describe("AberrationsPanel", () => {
  ...
  beforeEach(() => {
    ...
    const fieldCurvatureResult = withCurveFields({
    ...
    mockComputeFieldCurvature.mockReturnValue(fieldCurvatureResult);
    mockComputeFieldCurvatureBundle.mockReturnValue({
      fieldCurvature: fieldCurvatureResult,
      chromaticFieldCurvature: fieldCurvatureResult,
    });
  });
  ...
  it("forwards aberration-control state into spherical and field-curvature calculations", () => {
    ...
    expect(mockComputeFieldCurvatureBundle).toHaveBeenCalledTimes(1);
    expect(mockComputeFieldCurvatureBundle).toHaveBeenCalledWith(
      baseProps.L,
      baseProps.zPos,
      baseProps.focusT,
      baseProps.zoomT,
    ...
    expect(mockComputeFieldCurvature).not.toHaveBeenCalled();
  });
  ...
  it("keeps field curves uncapped while omitting out-of-circle astigmatism", () => {
    mockComputeSphericalAberration.mockReturnValue(makeSaResult(-0.012));
    const wideFieldCurvature = withCurveFields({
      fieldFractions: [0, 0.25, 0.5, 0.75, 1],
      usableFieldCount: 5,
      imagePlaneZ: 105,
    ...
    mockComputeFieldCurvatureBundle.mockReturnValue({
      fieldCurvature: wideFieldCurvature,
      chromaticFieldCurvature: wideFieldCurvature,
    });
