...
import { createSurfaceProfile, createTiltedPlaneProfile } from "../../../src/optics-2/math/surfaceProfile.js";
import { intersectSurfaceProfile } from "../../../src/optics-2/math/intersection.js";
import {
  cross,
  ...
  reflect,
  refract,
} from "../../../src/optics-2/math/vector.js";
import { OPTICS_2_PARITY_TOLERANCES } from "../parity/tolerances.js";

function expectClose(actual: number, expected: number, tolerance: number): void {
  expect(Math.abs(actual - expected)).toBeLessThanOrEqual(tolerance);
}

describe("Optics 2 vector math", () => {
...
describe("Optics 2 surface profiles", () => {
  it("matches legacy spherical and aspheric sag/slope formulas", () => {
    const R = 42.5;
    const h = 12.3;
    const spherical = createSurfaceProfile({ R }, undefined);
    expectClose(spherical.sag(h), sag(h, R), OPTICS_2_PARITY_TOLERANCES.surface.sag.abs);
    expectClose(spherical.slope(h), sagSlopeRaw(h, R, undefined), OPTICS_2_PARITY_TOLERANCES.surface.slope.abs);
    ...
    expectClose(aspheric.sag(h), conicPolySag(h, R, asphere), OPTICS_2_PARITY_TOLERANCES.surface.sag.abs);
    expectClose(aspheric.slope(h), sagSlopeRaw(h, R, asphere), OPTICS_2_PARITY_TOLERANCES.surface.slope.abs);
    expectClose(length(aspheric.normalAt(aspheric.pointAt(0, 3, 4), 0)), 1, 1e-12);
  });

  it("preserves the legacy conic denominator guard outside the real spherical domain", () => {
    const asphere: AsphericCoefficients = {
      K: 0,
      A4: 0,
      A6: 0,
...
describe("Optics 2 surface intersections", () => {
