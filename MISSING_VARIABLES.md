...
> **Status (2026-05-26):** Historical brainstorming. Several items have since moved forward; in particular, the
> chromatic engine now has a vendor Sellmeier/polynomial/power-series catalog plus measured-line-index fallback. Treat
> this file as future-idea context, not as the current capability inventory.
...
| 3 | **Full Spectral Dispersion Curve** | `sellmeier` — Sellmeier coefficients (B1, B2, B3, C1, C2, C3) per glass | Catalog entry, not `ElementData` | Implemented for catalog-resolved glasses through `src/optics/glassCatalog.ts`; unresolved proprietary glasses still need backfill. | Historical row retained for context. Current chromatic tracing prefers Sellmeier/vendor polynomial/power-series data, then measured line indices, then Abbe fallbacks. |
