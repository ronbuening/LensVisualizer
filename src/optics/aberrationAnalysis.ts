...
export interface SphericalAberrationResult {
  ...
  /** @deprecated Back-compat alias for `referenceIntercept`. */
...
export function computeSphericalAberration(
  ...
  if (referenceIntercept === null) return null;
  ...
  const saMm = realIntercept - referenceIntercept;
  return {
    saMm,
    saUm: saMm * 1000,
    realIntercept,
    referenceIntercept,
    paraxialIntercept: referenceIntercept,
  };
}
...
export function computeSAProfile(
  ...
  if (referenceIntercept === null) return [];
  ...
  for (const fraction of PROFILE_FRACS) {
    ...
    points.push({ fraction, saMm: realIntercept - referenceIntercept });
  }
