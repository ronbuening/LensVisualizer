...
const DesignGoalsDiagram = memo(function DesignGoalsDiagram({ isDark }: { isDark: boolean }) {
  ...
  return (
    ...
    >
      ...
      <text x="120" y="44" textAnchor="middle" fill={c.label} fontFamily={SA_FONT} fontSize={11}>
        soft subject
      </text>
      <text x="120" y="60" textAnchor="middle" fill={c.muted} fontFamily={SA_FONT} fontSize={10}>
        core + halo
      </text>
      <text x="320" y="44" textAnchor="middle" fill={c.label} fontFamily={SA_FONT} fontSize={11}>
        defocus bias
      </text>
      <text x="320" y="60" textAnchor="middle" fill={c.muted} fontFamily={SA_FONT} fontSize={10}>
        R / F exchange
      </text>
      <text x="520" y="44" textAnchor="middle" fill={c.label} fontFamily={SA_FONT} fontSize={11}>
        adjustable range
      </text>
      <text x="520" y="60" textAnchor="middle" fill={c.muted} fontFamily={SA_FONT} fontSize={10}>
        under – sharp – over
      </text>
      ...
      <text x="120" y="196" textAnchor="middle" fill={c.muted} fontFamily={SA_FONT} fontSize={10}>
        in-focus point
      </text>

      {/* Panel 2: foreground/background exchange with a concentrated subject */}
      <text x="320" y="100" textAnchor="middle" fill={c.accent} fontFamily={SA_FONT} fontSize={11}>
        R ↔ F
      </text>
      <circle cx="285" cy="140" r="24" fill="url(#sa09-edge)" />
      <circle cx="355" cy="140" r="24" fill="url(#sa09-center)" />
      <text x="285" y="182" textAnchor="middle" fill={c.muted} fontFamily={SA_FONT} fontSize={10}>
        front
      </text>
      <text x="355" y="182" textAnchor="middle" fill={c.muted} fontFamily={SA_FONT} fontSize={10}>
        behind
      </text>

      {/* Panel 3: under–sharp–over control */}
      <circle cx="450" cy="114" r="13" fill="url(#sa09-halo)" />
      <circle cx="450" cy="114" r="4" fill="url(#sa09-core)" />
      <circle cx="520" cy="114" r="5" fill="url(#sa09-core)" />
      ...
      <text x="450" y="172" textAnchor="middle" fill={c.muted} fontFamily={SA_FONT} fontSize={10}>
        under
      </text>
      <text x="520" y="172" textAnchor="middle" fill={c.muted} fontFamily={SA_FONT} fontSize={10}>
        sharp
      </text>
      <text x="590" y="172" textAnchor="middle" fill={c.muted} fontFamily={SA_FONT} fontSize={10}>
        over
      </text>

      {/* Example lenses */}
      <text x="120" y="240" textAnchor="middle" fill={c.muted} fontFamily={SA_FONT} fontSize={10}>
        e.g. Varisoft
      </text>
      <text x="320" y="240" textAnchor="middle" fill={c.muted} fontFamily={SA_FONT} fontSize={10}>
        e.g. DC-Nikkor
      </text>
      <text x="520" y="240" textAnchor="middle" fill={c.muted} fontFamily={SA_FONT} fontSize={10}>
        e.g. Portrait Heliar
      </text>
    </svg>
  );
