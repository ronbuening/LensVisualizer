...
import type { ComaPreviewFieldResult, ComaPreviewResult } from "../../optics/aberrationAnalysis.js";
...
function renderTile(field: ComaPreviewFieldResult, index: number, sharedRelativeHalfRangeMm: number, t: Theme) {
  ...
  const zeroX = plotX + PLOT_W / 2;
  const zeroY = plotY + PLOT_H / 2;
  ...
  return (
    <g key={field.label}>
      ...
      <text x={zeroX} y={tileY + TILE_H - 4} textAnchor="middle" fill={t.muted} fontSize={7.5} fontFamily="inherit">
        Chief-ray-centered image height (mm)
      </text>
    </g>
  );
}

export default function ComaPreviewGrid({ result, t }: ComaPreviewGridProps) {
  return (
    <svg viewBox={`0 0 ${VB_W} ${VB_H}`} style={{ display: "block", width: "100%", maxWidth: VB_W, height: "auto" }}>
      ...
      {result.fields.map((field, index) => renderTile(field, index, result.sharedRelativeHalfRangeMm, t))}
