...
The viewer offers two ray-launch modes, toggled in the control bar: **FROM ∞** and **TRACKS FOCUS**. In FROM ∞ mode, all on-axis rays enter parallel to the optical axis regardless of where the focus slider is set. This shows the *geometric* cone of light the lens produces for a distant object and makes it easy to see how the focal point drifts relative to the image plane as the internal spacings change. In TRACKS FOCUS mode, the rays are given an input convergence angle that corresponds to the current conjugate distance, so the traced bundle represents light actually arriving from a subject at that distance. The difference matters because aberrations — especially spherical aberration and coma — shift in magnitude and sign as the object distance changes. A lens that is well-corrected at infinity may show noticeably different aberration structure at its close-focus limit, and TRACKS FOCUS mode makes this visible. The coma preview in the Aberrations panel now reuses the same off-axis field geometry but traces a fixed circular pupil pattern with the skew-ray core. Each sample is projected to the image plane and plotted as chief-ray-centered tangential and sagittal image height, giving the preview a real 2D point cloud rather than a synthesized thickness. The **Bokeh Preview (Beta)** overlay extends that same tracing model into a dual subject-distance comparison: one 2x2 grid for infinity and one for minimum focus, each sampled at 0%, 25%, 50%, and 75% of the current half-field. Because both grids use the current aperture and the active focus state, they make it possible to see cat's-eye clipping, vignetting, and spherical-aberration shading change as you stop down or refocus. The dense meridional plot below the coma preview remains the direct diagnostic for reading the asymmetric tangential spread across the pupil, while the field-curvature / astigmatism sweep now solves its tangential and sagittal best-focus positions from that same shared off-axis tracing model.
