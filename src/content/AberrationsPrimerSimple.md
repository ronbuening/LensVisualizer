...
**Coma** affects off-axis points. Instead of forming a tight dot, rays from a point away from the center of the frame spread into a comet-shaped flare — brighter on one side, fading on the other. It arises when different zones of the aperture magnify the off-axis image by different amounts. In the viewer, switching on the off-axis ray bundle gives a rough sense of how symmetrically those rays converge: a well-corrected lens keeps them tightly grouped, while one with residual coma fans them out unevenly. The Aberrations panel now includes an estimated 2D coma preview at several field positions. Its horizontal spread comes directly from the real chief-ray-centered meridional footprint, while its vertical thickness is synthesized by expanding each valid meridional slice across the circular pupil chord it represents. That makes it a useful point-shape estimate rather than a full skew-ray spot diagram. The dense meridional coma plot below it remains the physically grounded diagnostic. **Astigmatism** is a related off-axis defect in which rays arriving in two perpendicular planes (tangential and sagittal) focus at different distances, so the image of a point becomes a short line rather than a dot.
