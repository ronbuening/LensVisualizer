# Carl Zeiss Biogon 21mm f/4.5 — Optical Analysis
...
The Biogon consists of **8 elements in 5 air-spaced groups** (referred to in the patent as "components" A through E). The structure is quasi-symmetric about the iris diaphragm:

- **Component A** (L₁): Negative meniscus, convex toward the object
- **Component B** (L₂): Negative meniscus, convex toward the object
- **Component C** (L₃ + L₄): Cemented doublet, net positive
- **Component D** (L₅ + L₆ + L₇): Cemented triplet, net positive
- **Component E** (L₈): Negative meniscus, convex toward the image

The iris diaphragm sits in the third air space — between Components C and D. This is the design's axis of quasi-symmetry: the front half (A–B) mirrors the rear half (E) in function, while the two positive compound groups (C and D) flank the stop.

### 2.2 Aspherical Surfaces

**There are no aspherical surfaces in any of the three patent examples.** All 13 optical surfaces are spherical. The patent text makes no mention of aspheric profiles, conic constants, or polynomial coefficients. This is consistent with the manufacturing technology of the early 1950s, when aspherical surface fabrication was prohibitively expensive for production photographic objectives. The remarkable aberration correction of the Biogon is achieved entirely through spherical surface bending, glass selection, and cemented group construction — a testament to Bertele's design skill.

The modern Zeiss C Biogon T* 21mm f/4.5 ZM (introduced 2004 for Leica M mount) also uses no aspherical elements, as denoted by its "C" (Classic) designation, though it employs a different optical configuration (8 elements in 6 groups).
...
| Angle of view | ~91.7° diagonal |
| Minimum focus distance | 0.9 m (some sources: 1.0 m) |
| Filter thread | 40.5 mm |
| Aperture blades | 8 (continuous, no click stops) |
...
| r₁₂ | −40.15 | 9.8 | 1.64200 | L₈ | E | L₈ front |
| r₁₃ | −102.64 | — | 1.0 (air) | — | — | Image space |

### 3.2 Patent Self-Consistency Checks

The patent text provides several derived quantities for Example 2. All were verified against the prescription:

| Quantity | Patent claim | Computed | Status |
|----------|-------------|----------|--------|
| Separation of convex outer surfaces C and D | 0.92f | 92.0 mm | ✓ |
| Cemented surface radius in C (r₆) | 0.254f | 25.37 mm | ✓ |
| Cemented surface convex toward image in D (r₁₀) | 0.255f | 25.51 mm | ✓ |
...
### 4.3 Component Focal Lengths

The following focal lengths were computed via thick-lens paraxial ray trace (standalone elements in air, and cemented subsystems treated as groups):
...
## 6. Element-by-Element Optical Role
...
The air space between L₁ and L₂ (l₁ = 13.2 mm) is itself optically significant: the patent notes that it takes "the shape of a collecting meniscus element with its convex outer surfaces turned towards the object." This air lens provides weak positive power that partially compensates the negative power of L₁ and L₂, improving the off-axis ray trajectory.

### 6.2 Component B — L₂ (Second Negative Meniscus)
...
### 6.3 Component C — L₃ + L₄ (Cemented Doublet)

**L₃ — Shape:** Meniscus, concave toward image; individually negative (f = −106 mm thick-lens).
...
Component C is the front half of the central positive core. The doublet structure performs several simultaneous functions:

1. **Spherical aberration correction:** The cemented surface at r₆ (R = +25.37 mm), where light passes from the high-index LaK10 (n = 1.721) into the lower-index SK7 (n = 1.607), produces a refractive index step of Δn = −0.113. This surface's curvature and index break are balanced to control the zonal structure of spherical aberration. The patent notes that keeping the radii of curvature of the cemented surfaces greater than 0.30f is important "in order to keep the zonal bulging of the spherical aberration curve sufficiently small."

2. **Chromatic correction:** Although both glasses are crowns (V > 50), the V-number difference (50.3 vs. 59.5) provides partial achromatic correction within the group.

3. **Coma control:** The strong meniscus form of L₃ (both surfaces convex toward the object) helps control coma for off-axis bundles by gradually bending oblique rays before they reach the powerful biconvex L₄.

The thick construction of L₃ (d₃ = 29.3 mm — nearly 30% of the design focal length) is a distinctive feature of the Biogon. This thickness creates significant principal-plane separation within the element, contributing to the Biogon's characteristic very short back focal distance.
...
1. **Achromatic correction:** The crown–crown–flint triplet produces well-corrected axial color. L₆ (the biconvex crown) carries the convergent power, while L₅ (crown, negative) and L₇ (flint, negative) split the divergent correction between them. The use of two negative elements flanking the positive one — rather than a simple doublet — provides an extra degree of freedom for simultaneously correcting both axial and lateral chromatic aberration.
...
**Shape:** Meniscus, convex toward the image; both surfaces concave toward the object.
**Power:** Moderate negative (f = −109 mm thick-lens).
**Glass:** LaK/SK-type dense crown (n_d = 1.642, V_d = 58.1).
...
## 7. Air Spaces as Optical Elements
...
- **l₃** (between C and D): This is the iris diaphragm location. At 4.7 mm (0.047f), it is deliberately kept small to maintain compactness while providing enough room for the physical iris mechanism. The patent states this spacing should be "greater than 0.03f" for adequate astigmatic correction.
...
At f = 21 mm and f/4.5, the hyperfocal distance is approximately 3.0 meters (for a circle of confusion of 0.033 mm). When focused at the hyperfocal distance, everything from about 1.5 meters to infinity is in acceptable focus. This explains why the production lens was sometimes described as a "practically fixed-focus lens for instantaneous snapshots" — for most photographic situations, the focus could be set once and left alone.

---

## 9. Design Philosophy: The Quasi-Symmetric Wide-Angle

The Biogon exemplifies a class of wide-angle designs based on the principle that a lens system perfectly symmetric about the aperture stop has exactly zero odd-order aberrations (coma, distortion, and lateral chromatic aberration) when operating at unit magnification (1:1). The Biogon never operates at 1:1 in practice — at infinity focus it operates at zero magnification — but the near-symmetry still suppresses these aberrations substantially across all conjugates. The suppression is imperfect (because the design is only quasi-symmetric and the magnification is not unity), but it is sufficient to produce what was, in 1954, an unprecedented combination of low distortion, low coma, and well-corrected lateral color at 90° field coverage.

The price of this architecture is the very short back focal distance. Because the design is non-retrofocus, the rear principal plane lies close to the image, and the rear element must physically protrude deep into the camera body. This makes the Biogon incompatible with SLR mirror mechanisms at standard focal lengths and restricts it to rangefinder cameras or SLRs with mirror lock-up.

### 9.1 Distortion

Distortion is an odd-order aberration and is therefore strongly suppressed by the quasi-symmetric layout. In a perfectly symmetric system at 1:1 magnification, distortion is exactly zero by symmetry. The Biogon operates at infinity focus (far from 1:1), but the near-symmetry still keeps distortion extremely low across the entire 90°+ field. Contemporary reviews and modern measurements consistently describe the Biogon's distortion as essentially negligible — a remarkable achievement for a 90° wide-angle lens and one of the primary reasons the design was adopted for aerial survey photography (as the Aviogon) where geometric fidelity is paramount.

### 9.2 Peripheral Illumination

The patent devotes significant attention to illumination at the image periphery, identifying it as a key design goal. In a conventional wide-angle lens, the illumination at the edge of the field drops off sharply according to the cos⁴θ law (where θ is the field angle), producing images that are dramatically darker in the corners. The Biogon mitigates this through two mechanisms:

First, the large negative front menisci (L₁ and L₂) act as field lenses that steer oblique ray bundles into the central positive groups with minimal vignetting. The patent states that this requires "sufficiently large free lens diameters" — the front elements are substantially wider than the stop diameter — and that the oblique pencils must pass "without vignetting" and without excessive coma.

Second, the entrance pupil of the lens effectively shifts off-axis for oblique fields (a consequence of the thick, strongly curved front elements), which increases the apparent aperture as seen from the edge of the field. This partially compensates the cos⁴θ falloff. The result is noticeably more even illumination across the frame than competing wide-angle designs of the era.

### 9.3 Relationship to Roossinov's Prior Art

The patent cites Roossinov (US 2,516,724, 1950) among its references. Michail Roossinov's 1946 design for a symmetric ultra-wide-angle lens — with negative menisci flanking positive doublets — established the architectural principle that Bertele refined. Bertele's contribution was to improve the aberration correction (particularly the zonal structure of spherical aberration and the lateral color balance) through the use of lanthanum glasses, the split front meniscus (two elements instead of one), and the cemented triplet in Component D.
...
1. **No aspherical surfaces.** The entire design uses only spherical surfaces — all corrections are achieved through glass selection, element bending, and cemented group construction.

2. **Glass selection is predominantly crown-based.** Seven of the eight elements use crown-type glasses (V_d > 50), spanning the FK, BK, LaK, SK, and BaK families. Only L₇ uses a dense flint (SF1-type, V_d = 29.3). This crown-heavy formulation is unusual for a lens of this complexity and contributes to the design's low chromatic aberration at wide field angles.
