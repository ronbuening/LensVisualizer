...
At infinity focus (F3s configuration), the total optical track from surface 1 to the image plane is 76.01 mm. The G1 group (surface 1 through the stop) spans 25.45 mm; the G2 group (stop through the image) spans 50.56 mm. The back focal distance is 15.00 mm.

### 2.4 Optical Conditions

The patent establishes conditions governing the system's quasi-symmetric balance. For Example 5:

**Condition 1 (DF3/DR3):** The ratio of the distance from the stop to the object-side face of the 3rd element counting outward on each side. DF3 = 19.77 mm (stop to surface 5, the object-side face of L13), DR3 = 18.87 mm (stop to surface 16, the image-side face of L23). This yields DF3/DR3 = 1.05, confirming the patent's stated value and satisfying the constraint 0.55 < DF3/DR3 < 1.35. The ratio slightly exceeds 1.0, indicating that the G1 group extends modestly farther from the stop than the G2 group — consistent with the need to handle the long object conjugate.

Note: The L13 designation refers to the 3rd element from the object, which is also the 3rd counting outward from the stop on the object side — the two numbering conventions coincide for the G1 group of Example 5.
...
```
 G1 Group 101                      G2 Group 102
 (5 elements, 5 groups)               (5 elements, 3 groups)
 OBJ → [L11] [L12] [L13] [L14] [L15] |STO| [L21|L22] [L23|L24] [L25] → IMG
       (−)  (+)   (+)   (+)   (−)          (−)(+)    (+)(−)    (−)
...
The G1 group consists entirely of single (air-spaced) elements. The G2 group uses two cemented doublets (Jb and Ja) followed by an isolated aspheric field corrector (L25), separated by a 12.49 mm air gap.

### 3.2 Partial-Symmetry Groups (Gc)

The patent's central innovation is the "partial-symmetry lens group" — a sub-assembly of two or more positive lenses flanked by negative lenses, whose local symmetry provides aberration cancellation. In Example 5, the G1 group forms one Gc with the power sequence (−)(+)(+)(+)(−), and the G2 group forms another with (−)(+)(+)(−) in the cemented doublets. These Gc groups enable the quasi-symmetric cancellation of coma, distortion, and lateral chromatic aberration between the G1 and G2 halves of the system.
...
### Element 2 — L12: Positive Meniscus, Double-Sided Aspheric
...
**Optical function.** The primary monochromatic correction element and the most technically demanding component in the G1 group. At nd = 1.852, this dense lanthanum glass provides substantial refractive power without excessive curvature. The meniscus form — both surfaces curving toward the object — equalizes incidence angles on both faces, which the patent explicitly identifies as a strategy for reducing error sensitivity (¶42). The element sits immediately after the negative L11, receiving the diverging beam and beginning the convergence toward the stop.
...
### Element 3 — L13: Biconvex Positive
...
**Optical function.** The secondary chromatic correction element and power-sharing partner to L13. K-GFK68 is a Sumita fluorophosphate crown ("ガラス蛍りん酸クラウン") with a higher refractive index than S-FPL51 (1.593 vs 1.497), allowing it to achieve more optical power per unit curvature. Its anomalous partial dispersion (+0.0195) is smaller than S-FPL51's (+0.0376) but still substantial. Together, L13 and L14 form a graded chromatic correction pair: L13 handles the bulk of the secondary spectrum correction while L14 supplements it with additional converging power on gentler curves.
...
### Element 5 — L15: Negative Meniscus
...
**Optical function.** The closing bracket of the front partial-symmetry group and the last element before the stop. Its position is critical: the large air gap following it (ZD10 = 5.49 mm at infinity, the widest in the G1 group) serves as the primary focus adjustment space in several of the patent's focus schemes. This element is the boundary between the G1 group and the stop, meaning it controls the exit angle of the converging beam entering the stop region.
...
### Element 6 — L21: Biconcave Negative (cemented with L22 to form doublet Jb)
...
### Element 7 — L22: Biconvex Positive (cemented with L21 = doublet Jb)
...
### Element 8 — L23: Biconvex Positive (cemented with L24 = doublet Ja)
...
### Element 9 — L24: Biconcave Negative (cemented with L23 = doublet Ja)
...
**Optical function.** The closing negative of the rear partial-symmetry group. Its low refractive index creates the large index step with Element 8 (Δnd ≈ 0.249) that drives spherical aberration correction in doublet Ja. The fact that this element is not APD is itself informative: it tells us the G2 group's chromatic work is concentrated entirely in doublet Jb (which contains Element 6, inferred APD for the E-mount), while doublet Ja is reserved for monochromatic correction using conventional glasses.
...
### Element 10 — L25: Negative Meniscus, Double-Sided Aspheric
...
```
 ★  Elem 1 (L11)  — Front of system. Negative. Full field angle exposure.     [inferred APD]
    Elem 2 (L12) — Aspheric, normal glass.
 ★  Elem 3 (L13) — S-FPL51, positive. Primary chromatic powerhouse.          [patent-listed dPgF]
 ★  Elem 4 (L14) — K-GFK68, positive. Secondary chromatic powerhouse.        [patent-listed dPgF]
 ★  Elem 5 (L15) — Negative meniscus. Last before stop.                      [inferred APD]
    ═══ STOP ═══
 ★  Elem 6 (L21) — Negative biconcave. First after stop.                     [inferred APD]
    Elem 7 (L22) — Normal LAH glass, positive.
    Elem 8 (L23) — Normal LAH glass, positive.
    Elem 9 (L24) — Normal glass, negative.
    Elem 10 (L25) — Aspheric, BK7-type.
```

Four of the five APD elements are in the G1 group; only one (Element 6) is in the rear. No APD elements appear in the G2 group beyond the first post-stop element. This asymmetry reflects two deliberate design choices:

First, the G1 group is where the beam is widest and the marginal ray heights are largest. Since axial chromatic aberration depends on the product of element power and marginal ray height squared, the G1 group dominates the axial chromatic correction. Placing three of the five APD elements here (Elements 1, 3, and 4) maximizes their chromatic impact.
...
The G2 group beyond Element 6 uses only conventional glasses, confining its role to monochromatic aberration correction via the doublets and the aspheric field corrector.
...
|---|---|---|---|---|---|---|
| 1 (L11) | 1.54373 | 47.65 | ? | ★ (inferred) | Unmatched | — |
| 2 (L12) | 1.85249 | 42.08 | — | — | S-LAH79 (OHARA) | Probable |
| 3 (L13) | 1.49700 | 81.61 | +0.0376 | ★ (patent) | S-FPL51 / K-PFK80 (universal) | Certain |
| 4 (L14) | 1.59282 | 68.62 | +0.0195 | ★ (patent) | K-GFK68 (Sumita) | Certain |
| 5 (L15) | 1.51322 | 57.22 | ? | ★ (inferred) | Unmatched | — |
| 6 (L21) | 1.70269 | 34.87 | ~−0.01 (est.) | ★ (inferred) | Unmatched (KZFS-adjacent) | — |
| 7 (L22) | 1.79334 | 47.18 | — | — | S-LAH64 (OHARA) | Possible (weak) |
| 8 (L23) | 1.80258 | 46.60 | — | — | S-LAH65V (OHARA) | Probable |
| 9 (L24) | 1.55362 | 45.38 | — | — | Unmatched | — |
| 10 (L25) | 1.51633 | 64.06 | — | — | S-BSL7 / N-BK7 (universal) | Certain |
...
7. **The G2 group performs a clean division of labor:** doublet Jb provides chromatic correction (large Δνd), doublet Ja provides monochromatic correction (large Δnd, negligible Δνd). This is inferred from the numerical data, supported by the patent's general discussion of these principles (¶19, ¶44).
