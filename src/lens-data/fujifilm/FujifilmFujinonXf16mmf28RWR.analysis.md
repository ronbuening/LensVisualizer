...
The patent calls the positive / positive / negative arrangement a “telephoto type configuration” in ¶0045. Under the Lens Patent Decomposition taxonomy, however, “telephoto” is reserved for `TL/EFL < 1`. The physical model track, surface 1 to the image plane including the 2.85 mm plate PP, is 60.49 mm, giving `TL/EFL = 3.670555886` (3.611667894 with the plate's air-equivalent path), so the data is not classified as telephoto. It is likewise not retrofocus under the project criterion: the independently computed Gaussian back focal distance is 14.082318275 mm, giving `BFD/EFL = 0.854520354`, below unity. The authored surface-19-to-image path (11.10 mm air, the 2.85 mm plate, 1.10 mm air) has an air-equivalent length of 14.079538095 mm; that is the patent's rounded image-plane spacing rather than the Gaussian BFD. The patent's descriptive terminology is therefore preserved as a source statement but not adopted as the project architecture label.
...
Independent reduced-angle and ABCD calculations performed from the final TypeScript surface arrays give an effective focal length of 16.479792674 mm and an air-equivalent Gaussian back focal distance of 14.082318275 mm, reproducing Example 3 Table 10 at its 0.01 mm precision. The corresponding principal-plane offsets are +22.852720796 mm from surface 1 for the front principal plane and -2.397474399 mm from surface 19 for the rear principal plane under the model sign convention. The modeled stop and entrance-pupil geometry give FNo 2.880000235, reproducing the patent's FNo 2.88.

The patent's parallel plate PP is not a lens element. Paragraph 0039 identifies it as a no-power filter/cover-glass surrogate. The data models it in `rearPlates`, so every analysis traces it but the diagram does not draw it: surface 19 keeps the patent's 11.10 mm gap, followed by the 2.85 mm plate (`nd = 1.51633`, `νd = 64.14`, θgF 0.53531) and 1.10 mm of air to Sim. The model image plane is therefore the patent's physical Sim plane; the paraxial equivalent of that rear path is 14.079538095 mm of air.
