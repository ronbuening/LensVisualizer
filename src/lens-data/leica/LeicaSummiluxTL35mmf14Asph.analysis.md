...
The model retains 12 physical elements, 8 air-separated groups, and 4 aspherical surfaces. The patent's rear plane-parallel plate PT is not counted among the elements, because ¶0036 identifies it as equivalent to sensor cover glass and low-pass filtering rather than as part of the lens proper. It is modeled in `rearPlates` as described below: every analysis traces it, but it is not drawn.
...
The patent's rear PT plate, which the patent identifies as the sensor-cover/low-pass equivalent (¶0036), is modeled in `rearPlates` rather than as a lens element: surface 21 keeps the patent's **18.13 mm** gap to PT, followed by the 1.50 mm plate (nd 1.5168, νd 64.20, N-BK7 class, published effective radius 16.000 mm) and the patent's 0.80 mm bf to the image. Every analysis traces the plate, but it is not drawn. Its paraxial air equivalent after surface 21 is 18.13 + 1.50 / 1.5168 + 0.80 = **19.918924 mm**, so the first-order d-line behavior is unchanged from an air-equivalent model at both published focus endpoints. The modeled physical first-surface-to-image track is **94.76 mm**, consistent with the patent's printed TL of 94.8 mm that includes the plate.
