...
$n_d = 1.75000$, $\nu_d = 25.14$. Glass: `750251` dense/fluor flint. Standalone $f = -21.02$ mm.
...
The prior analysis described this glass as SFS1 / FDS1 class. That is too specific and is not supported by the current catalog check. The patent values correspond to code `750251`. Current HOYA cross-reference data lists FF8 at 752/251, which is close in dispersion but not an exact $n_d$ match. The data file therefore uses a code-based dense/fluor flint annotation rather than a false catalog identification.
