...
Surfaces i=9 (R=47.82) and i=10 (R=14.26). The strongest negative element in G1 and the element with the smallest clear aperture in the front group (Hs = 10.54 mm). The steep rear surface (R=14.26) produces intense divergence, which is essential for positioning the rear principal point of the entire system forward — the key mechanism for achieving a short TTL relative to focal length. The S-TIH14 glass has the highest dispersion in G1, maximizing the chromatic correction from this compact, strongly curved element.
