...
The patent calls G5 fixed relative to the image surface (¶0128), but Table 3's back-focus values imply a small image-relative variation. After rear-plate normalization, the modeled G5 station changes by about 0.0175 mm across the three states. The numerical table is retained rather than forced to agree with the qualitative fixed-unit statement.

Under this project's explicit definitions, the design is not labeled either telephoto or retrofocus at any published zoom state. The final first-vertex-to-image track divided by EFL is 12.79, 6.32, and 3.82 from wide to tele, all greater than unity; the corresponding powered-stack back focal distance divided by EFL is 0.332, 0.179, and 0.094, all less than unity. These are classification tests only and do not replace the patent's own description of the zoom architecture.
...
The final LensVisualizer model contains 22 modeled surfaces, including one `STO`, after removal of the source rear plate and collapse of the adhesive plane. It retains all 11 physical lens elements.

At wide, middle, and tele, independent reduced-angle sequential tracing and ABCD matrix multiplication agree to better than 1e−12 in the system matrix. The final EFL values are 4.886165571 mm, 9.122566195 mm, and 17.194509683 mm. The per-state air rear spacings after the omitted plate are 1.621328395 mm, 1.636948819 mm, and 1.619439804 mm.
