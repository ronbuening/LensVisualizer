...
The retrofocus classification is also numerically satisfied. The independently computed air-equivalent back focal distance from the last active rear vertex is 24.772754 mm (25.624547 mm physically, through the 2.500 mm rear plate), greater than the 14.540815 mm EFL, giving BFD/EFL = 1.703670. The air-equivalent first-vertex-to-image track is 104.511934 mm (105.363727 mm physical with the plate), so track/EFL = 7.187488 (7.246068 physical); the model is therefore not telephoto under the project's `track/EFL < 1` criterion. The patent explicitly discusses a retrofocus construction and the use of a negative first group for wide-angle coverage (¶0047).
...
The patent also includes a plane-parallel rear optical element at surfaces 26-27, described generically as a filter or cover glass. The 2.500 mm plate (nd = 1.51680, νd = 64.20, modeled as N-BK7) is carried in `rearPlates`: it is traced by every analysis but not drawn in the section or listed among the 14 elements. The last lens surface stores the patent's fixed D3 = 22.66 mm to the plate. Because the source maintains D4 + D5 = 0.500000 mm while D5 becomes negative at finite focus, D5 is treated as a reference/bookkeeping coordinate rather than a physical propagation path, and the plate-to-image gap is the fixed sum 0.500 mm (derived, not printed as a single value). The paraxially equivalent air path is 22.66 + 2.500/1.51680 + 0.500 = 24.808206751 mm, the value earlier versions folded into the last gap. This is a reference-plane normalization; it does not add or reconstruct a focus motion.
...
The final data arrays compute an infinity EFL of **14.540814966 mm**, compared with the patent's stated 14.537406 mm. The first principal plane is **31.006477362 mm** imageward of the first active vertex and the second principal plane is **10.231938957 mm** imageward of the last active vertex under the model's signed convention. The air-equivalent paraxial BFD from the last active vertex is **24.772753923 mm** (**25.624547172 mm** physical through the rear plate). The air-equivalent first-vertex-to-image track is **104.511933751 mm**; the physical track including the 2.500 mm plate is **105.363727000 mm**, longer by 2.500 × (1 − 1/1.51680) = 0.851793 mm.
...
The computed Petzval sum is **+0.005289519 mm⁻¹**. Surface-by-surface accumulation uses $\phi/(nn')$, consistent with the project convention. Independent reduced-angle `[y,nθ]` and conventional `[y,θ]` ABCD formulations agree to machine precision for the transcribed infinity model. BFD/EFL = **1.703670** (air-equivalent), satisfying the project's retrofocus test, while track/EFL = **7.187488** (7.246068 physical), so the design is not telephoto.
