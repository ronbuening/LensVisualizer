...
Example 3 is a three-functional-group wide-angle design. From object to image, the groups are a **negative G1**, a **positive G2**, and a **positive G3**. The patent states that G1 remains fixed during focusing while G2 and G3 move independently toward the object when focusing from infinity to the nearest published distance (¶0048, ¶0062–¶0066). The numerical prescription confirms those power signs.

The isolated d-line group focal lengths computed from the final prescription are approximately −182.843184 mm for G1, +88.736798 mm for G2, and +50.896853 mm for G3. These are standalone group powers in air, not a statement that the groups retain those focal lengths unchanged when embedded in the complete system.

G1 contains five air-separated spherical elements. Its three leading negative elements expand the field and establish the long back-focus geometry, after which two positive elements partly recover convergence before the moving rear system. G2 consists of a positive singlet followed by a cemented positive/negative pair. G3 begins immediately behind the stop with a double-sided aspherical negative singlet, continues through a cemented positive/negative pair, and ends with a double-sided aspherical positive element.
...
The 497816 coordinate is not assigned to a particular vendor in the final data. It is also not equated directly with Samyang's marketed “ED” element because the patent does not make that production-material identification.

### L3 (L33) — Biconcave Negative

**nd = 1.75520, νd = 27.53. Glass: 755275 — dense-flint class (vendor unspecified). Standalone f = −59.143577 mm.**

L3 is the strongest standalone negative element in G1 by focal-length magnitude. Its biconcave form reinforces the front-group divergence after L1 and L2 and helps establish the ray-height expansion needed for a wide field with a long rear clearance.

Its relatively high index and low Abbe number contrast with L2's low-dispersion glass. The pair contributes a broad dispersion spread within G1, but the available data do not support a claim of anomalous partial dispersion or apochromatic correction.
...
L4 is the first positive element after the three leading negatives. Its role is to begin recovering convergence while G1 remains net negative as a subsystem. The very high refractive index allows meaningful positive power with one plane surface and a moderate curved rear surface.
...
L5 is the final element of fixed G1. It adds positive power after L4 but does not reverse the group's overall sign; the isolated five-element G1 remains negative at −182.843184 mm. Its plane rear surface also creates a simple reference boundary immediately before the variable G1–G2 spacing.

The very low Abbe number makes L5 a strong-dispersion component. The design consequently uses a wide range of dispersions within G1 rather than relying on a single low-dispersion material to control color.
...
L6 is the front singlet of moving G2 and supplies the principal positive action ahead of the cemented L7/L8 pair. It converts the divergent bundle leaving G1 into a more strongly converging bundle approaching the stop.
...
### L7 (L73) / L8 (L83) — Cemented Positive/Negative Pair D1
...
L7 and L8 share the cemented interface at surface 14. Their standalone powers have opposite signs and their Abbe numbers differ substantially. The cemented pair by itself has a computed net focal length of −133.002163 mm, even though L7 is a strong positive element. This negative cemented contribution follows the positive L6 singlet, leaving the complete G2 positive at +88.736798 mm.
...
### L10 (L103) / L11 (L113) — Cemented Positive/Negative Pair D2
...
L10 and L11 share the cemented interface at surface 20. Unlike D1, this positive/negative pair is net positive in isolation, with a computed cemented focal length of +97.612214 mm. It therefore contributes positive relay power within G3 after the negative L9 singlet.

The 497816/593354 dispersion contrast again provides conventional crown/flint color balancing. The design data do not identify either element as Samyang's marketed ED or HR material, so those production labels are not assigned here.
...
L12 is the final glass element and a strong positive member of G3. Both surfaces, 22A and 23A, are aspherical. The patent specifically states that an aspheric lens at the image-side end of G3 can be used to correct astigmatism and distortion (¶0049), which makes L12 the field-correction counterpart to the stop-adjacent L9 asphere.
...
| 497816 — low-dispersion crown class | 1.49700 | 81.61 | L2, L10 | Very low-dispersion crown coordinate used in both G1 and G3 |
...
| 835427 — high-index glass class | 1.83481 | 42.72 | L6 | High-index positive G2 singlet |
| 713539 — lanthanum crown class | 1.71300 | 53.94 | L7 | Lower-dispersion member of cemented pair D1 |
| 728283 — flint class | 1.72825 | 28.32 | L8 | Higher-dispersion negative member of D1 |
| E-FD8 catalog-equivalent | 1.68863 | 31.19 | L9 | Compatible dispersion proxy; production identity unspecified |
| 593354 — flint class | 1.59270 | 35.45 | L11 | Negative member of cemented pair D2 |
| L-LAH85V catalog-equivalent | 1.85343 | 40.54 | L12 | High-index final positive aspheric element |

Samyang's production specification describes the lens as using two aspherical, three HR, and one ED element. The patent directly establishes the two aspherical elements in Example 3, but it does not label individual elements with Samyang's HR or ED marketing categories. In particular, the 1.49700/81.61 coordinate appears twice, so assigning a single production “ED” designation to either L2 or L10 would be speculative.
...
The patent uses a published two-state floating internal-focus mechanism. G1 is fixed; G2 and G3 move independently toward the object as focus moves from infinity to the nearest published state (¶0048, ¶0062). The data file transcribes only those two endpoints and does not invent intermediate trajectories or reversals.

| Authored variable spacing | Infinity | Nearest published state | Change |
|---|---:|---:|---:|
| G1→G2 gap after surface 10 | 6.795765 mm | 1.000000 mm | −5.795765 mm |
| G2→STO gap after surface 15 | 7.732498 mm | 6.565902 mm | −1.166596 mm |
| Surface 23A→IMG, air-equivalent | 38.162369751 mm | 45.168708751 mm | +7.006339000 mm |

From the front-side gap changes, G2 moves 5.795765 mm objectward and G3 moves 6.962361 mm objectward. G3 therefore moves 1.166596 mm farther toward the object than G2 over the published endpoint range. The rear air-equivalent gap changes by a slightly different amount because it also incorporates the published change in the omitted plate-to-image spacing; it should not be interpreted as a direct mechanical travel measurement for G3.
...
The first two expressions require a source-interpretation note. Paragraphs 0069 and 0073 describe `D2i` and `D2n` as the distance from the rear vertex of G2 to the stop. Read literally, Example 3 would produce 0.087140 and 1.177675, which do not match Table 10. The tabulated 0.207 and 1.068 values are reproduced only when the distance is taken from the rear G2 vertex at surface 15 through the stop to the first G3 vertex at surface 17A, i.e. `D2 + 10.641 mm`. The final data preserves the published spacings; the analysis records this source-definition inconsistency rather than silently altering the prescription.

For the remaining expressions, `Rfront` is the 68.013 mm object-side radius of L6, `f2` is the computed +88.736798 mm G2 focal length, `frear` is the standalone −34.600387 mm focal length of the stop-adjacent L9, and `f3` is the computed +50.896853 mm G3 focal length.
...
- Paragraph 0047 calls G1 positive while also describing it as diverging; the claims, Example 3 description, and computed power all establish G1 as negative.
- Paragraph 0065's generic three-element cemented wording for part of G3 does not match Example 3 Table 7 or Figure 5, which show L9 air-separated from the cemented L10/L11 pair.
