...
The lens is a 5-group, 7-element design described by the patent and by Sato as a hybrid Gauss-Sonnar structure. The G1 group (ahead of the diaphragm) incorporates Sonnar-like characteristics — specifically the decomposition of the traditional cemented triplet into separate positive elements flanking a divergent air lens — while the G2 group follows a conventional Gauss doublet + singlet arrangement.

### Scaled Prescription (Embodiment 1 × 85)

| Surface | R (mm) | d (mm) | nd | vd | Medium | Element |
|---------|-------:|-------:|---:|---:|--------|---------|
| r1 | +66.436 | 7.497 | 1.77279 | 49.4 | → glass | L11 front |
| r2 | +346.596 | 1.496 | 1.0 | — | → air | L11 rear |
| r3 | +37.128 | 9.699 | 1.69680 | 55.6 | → glass | L12 front |
| r4 | +96.501 | 1.496 | 1.0 | — | → air | L12 rear |
| r5 | +167.280 | 6.503 | 1.78470 | 26.1 | → glass | L13 front |
| r6 | −120.003 | 2.797 | 1.75520 | 27.5 | → glass | L13→L14 junction |
| r7 | +23.639 | 19.499 | 1.0 | — | → air | L14 rear |
| *(diaphragm)* | — | — | — | — | — | *aperture stop* |
| r8 | −31.408 | 1.496 | 1.58144 | 40.8 | → glass | L21 front |
| r9 | +79.807 | 8.704 | 1.74443 | 49.4 | → glass | L21→L22 junction |
| r10 | −39.806 | 0.400 | 1.0 | — | → air | L22 rear |
| r11 | +106.004 | 4.803 | 1.74443 | 49.4 | → glass | L23 front |
| r12 | −322.405 | 42.628 | 1.0 | — | → air | L23 rear → image |

**Total optical track** (front of L11 to image plane): **107.01 mm**
...
|---------|---:|---:|------|-------------|---------------------------|---------|
| L11 | 1.77279 | 49.4 | 773-494 | Lanthanum dense crown (LaK–LaF border) | HOYA TAF1 / Schott N-LAF34 / OHARA S-LAH66 (773-496) | ≈0/0.2 |
| L12 | 1.69680 | 55.6 | 697-556 | Lanthanum crown (LaK) | HOYA LAC14 / Schott N-LAK14 / OHARA S-LAL14 (697-555) | ≈0/0.1 |
| L13 | 1.78470 | 26.1 | 785-261 | Dense flint (SF) | HOYA FD110 / Schott SF11 / OHARA S-TIH11 (785-257) | ≈0/0.3 |
| L14 | 1.75520 | 27.5 | 755-275 | Dense flint (SF) | HOYA E-FD4 / Schott SF4 / OHARA S-TIH4 (755-275) | exact/≈0 |
| L21 | 1.58144 | 40.8 | 581-408 | Light flint (LF) | HOYA E-FL5 / Schott LF5 / OHARA S-TIL25 (581-409) | ≈0/0.1 |
| L22 | 1.74443 | 49.4 | 744-494 | Lanthanum flint (LAF/LAM) | HOYA NBF1 / Schott (N-LAF35) / OHARA S-LAM60 (743-493) | 0.001/0.1 |
| L23 | 1.74443 | 49.4 | 744-494 | Lanthanum flint (LAF/LAM) | Same as L22 | — |

**Notes on specific identifications:**

**L11** sits right on the boundary between lanthanum crown (LaK, typically vd > 50) and lanthanum flint (LaF, typically vd < 50). The HOYA TAF1 / Schott N-LAF34 family (code 773-496) is the closest cataloged equivalent, with a Δvd of only 0.2. Both the patent value and catalog value are essentially at the LaK–LaF boundary at this index.

**L13** matches Schott SF11 in refractive index to five decimal places (1.78470 vs 1.78472) but differs in Abbe number by approximately 0.3–0.4 (26.1 vs 25.7–25.8). This may reflect the specific dispersion of a 1970s-era melt or a slightly different formulation that has since been discontinued. The family identification as dense flint is unambiguous.

**L22/L23** use identical glass. The nearest catalog match is the HOYA NBF1 / OHARA S-LAM60 family (code 743-493), with Δnd ≈ 0.001. These are lanthanum flint glasses with moderate dispersion — not lanthanum crowns, despite the relatively high Abbe number of 49.4.
...
**The L3 cemented doublet uses two dense flints with very similar Abbe numbers** (vd = 26.1 and 27.5, a difference of only 1.4). This is the key insight behind patent conditions (4)–(6). The cemented surface r6 has a weak positive refractive power contribution — computed as (n₄ − n₃)/r₆ = (1.75520 − 1.78470)/(−1.4118) = +0.0209φ — that generates high-order negative spherical aberration. Because the dispersion of the biconcave element (L14, vd = 27.5) is only slightly lower than the biconvex element (L13, vd = 26.1), this chromatic splitting is tuned to selectively increase negative spherical aberration at short wavelengths (g-line, λ = 435.8 nm) relative to the standard d-line (λ = 587.6 nm). This counteracts the over-correction of g-line spherical aberration that arises from the strong negative power of L3.

**The front elements (L11, L12) use high-index lanthanum glasses** (nd = 1.773 and 1.697 respectively) that provide strong positive power with relatively gentle curvatures, limiting the generation of high-order aberrations.

**L22 and L23 use identical glass** (nd = 1.74443, vd = 49.4), simplifying manufacturing logistics.

---

## 5. Element-by-Element Optical Role

### Group 1: L11 — Front Positive Meniscus
...
L11 is the first element the light encounters. As the widest element in the system — handling the full f/1.4 entrance pupil cone — it provides the initial convergence. Condition (1) prescribes 0.75φ < φ₁ < 0.85φ. Exceeding the upper bound would generate excessive annular (zonal) spherical aberration. Falling below the lower bound would reduce convergence and make it impossible to achieve the short back focal length required for compactness.

### Group 2: L12 — Second Positive Meniscus
...
L12 has a higher thin-lens power than L11 (0.981φ vs 0.799φ). The patent explicitly states that φ₂ > φ₁ is desirable: allotting more refractive power to the second group, which is immediately followed by a divergent air lens, allows the air lens to more effectively negate the high-order negative spherical aberration and astigmatism that L11 and L12 generate. Condition (2) prescribes 0.9φ < φ₂ < 1.1φ.

L12 uses a lanthanum crown with notably lower dispersion than L11 (vd = 55.6 vs 49.4), reducing chromatic aberration contributions from this more powerful positive element.

### The L12–L3 Air Lens (Divergent)

The air space between L12 (rear surface r4) and L3 (front surface r5) functions as a divergent air lens with net power φₐ = −0.215φ, satisfying condition (3): 0 < −φₐ < 0.3φ. This meniscus-shaped air lens is what Sato describes as Takiguchi's key invention — transforming the Sonnar's cemented triplet into a convex element + air lens + convex-concave doublet arrangement that clarified the optical role of each component and dramatically expanded design freedom.

### Group 3: L3 — Cemented Negative Meniscus (L13 + L14)

| Property | L13 (biconvex) | L14 (biconcave) |
|----------|---------------|----------------|
...
**Stop position estimation:** The patent does not specify the exact axial position of the stop within the d7 air gap (19.50 mm). The position was inferred from FIG. 1 at approximately 65% through the gap, yielding a pre-stop distance of 12.67 mm (r7 to stop) and post-stop distance of 6.83 mm (stop to r8). At this position, the paraxial marginal ray height is 16.7 mm, consistent with f/1.4 operation. The post-stop distance is constrained by the sag of the concave L21 front surface (r8), which extends 6.4 mm toward the stop at sd = 19.0 mm, leaving only 0.4 mm clearance.

### Group 4: L4 — Cemented Meniscus Doublet (L21 + L22)
...
**Embodiment 4 variant:** Embodiment 4 uses a structurally different L4. In that variant, L22 becomes a negative meniscus (R9 = −0.7647, R10 = −0.3953) and both L21 and L22/L23 use different glasses. This represents a fundamentally different rear-group correction strategy, which is why Embodiment 4 has its own cross-section (FIG. 5).

### Group 5: L23 — Rear Positive Singlet
...
L23 is separated from L4 by a thin air gap (d₁₀ = 0.40 mm at infinity) that widens during close focusing — this is the CRC variable gap. L23 provides the final positive convergence needed to bring the image to focus.

---

## 6. Focusing Mechanism: Close-Range Correction (CRC)

The patent describes the focusing method: the entire lens moves forward for close focus (unit focus), while simultaneously L11–L4 move by a slightly larger amount than L23, enlarging the d₁₀ air gap. This is Nikon's **Close-Range Correction (CRC)** system — the first application of CRC to a telephoto lens. The single variable gap creates a simple two-group floating system that compensates for field curvature, coma, and astigmatism shifts at close focus distances. Sato's analysis confirms that even at the 0.85 m minimum focus distance (magnification −1/7.9×), astigmatism remains well corrected with only moderate outer coma at the field edges.
...
1. **Edge thickness positivity:** For each element, the sag difference between front and rear surfaces must not exceed the center thickness. All elements pass, with L11 (0.76 mm) and L22 (0.77 mm) being the tightest.
...
4. **Cross-gap overlap:** For each air gap, the combined sag intrusion from adjacent surfaces does not exceed the gap thickness. The tightest clearances are the L12–L3 air gap (0.08 mm), the pre-stop gap (0.81 mm), and the post-stop gap (0.43 mm).
...
|---------|------|--------:|---------------------|
| r1 | L11 front | 32.0 | 72mm filter; L11 edge thickness |
| r2 | L11 rear → air | 31.0 | Element consistency |
| r3 | L12 front | 28.0 | L12 edge thickness |
| r4 | L12 rear → air | 25.0 | L12–L3 gap clearance |
| r5 | L13 front | 24.5 | L12–L3 gap clearance |
| r6 | L3 junction | 23.0 | L14/r7 sd/|R| cascade |
| r7 | L14 rear → air | 20.5 | sd/|R| = 0.867; pre-stop gap sag |
| STO | Aperture stop | 16.7 | Paraxial marginal ray height at 65% d7 |
| r8 | L21 front | 19.0 | Post-stop gap sag clearance |
| r9 | L4 junction | 19.0 | Element consistency |
| r10 | L22 rear → air | 20.0 | L22 edge thickness |
| r11 | L23 front | 22.0 | Off-axis beam containment |
| r12 | L23 rear → air | 21.5 | Element consistency |
...
Sato's *Thousand and One Nights* account provides the most authoritative description of the design philosophy. Takiguchi's key innovation was the decomposition of the Sonnar G1 group's cemented triplet (convex-convex-concave) into a structure with clear, separated optical roles:

**Traditional Sonnar front:** Convex₁ — Convex₂ — Concave (cemented triplet)

**Takiguchi's hybrid front:** Convex₁ (L11) — *divergent air lens* — Convex₂-Concave (L3, cemented doublet)

The air lens provides divergent power (φₐ = −0.215φ) that partially negates high-order aberrations from L11/L12, reduces the burden on L3, and creates design freedom impossible with the cemented triplet. Behind the diaphragm, L4 and L23 follow the conventional Gauss rear-group arrangement. The result combines the Sonnar's advantages (stopper surface at r7, sagittal coma reduction) with the Gauss structure's advantages (symmetry for distortion and lateral chromatic aberration correction).
