...
R₁ = +915.083, R₂ = +29.841 (both positive; R₁ ≫ R₂ → concave toward object). FL ≈ −35 mm. Glass: S-LAH58 (OHARA), dense lanthanum glass matching code 883/408. This is the strongest individual negative element in the variator. The meniscus shape (concave toward the object) is the Petzval-favorable orientation for a negative element.
...
Element 18 (negative meniscus, GMo aspherical): nd = 1.85400, νd = 40.4. The glass is **OHARA L-LAH85V**, a low-softening-temperature lanthanum dense flint matching code 854/404 — the "L-" prefix designates PGM compatibility, consistent with this element being glass-moulded. R₁ = +85.354, R₂ = +34.925 (FL ≈ −71 mm individually). Surface 32 carries the aspherical profile (K = 0, polynomial to 8th order; all coefficients negative). Element 19 (biconvex): nd = 1.60311, νd = 60.6, matching Schott N-SK14's code 603/606 constants. The cemented pair is net positive (+96 mm), with element 19 (FL ≈ +41 mm) overpowering element 18's divergence. The GMo aspherical surface on element 18 is the primary corrector for spherical aberration in the strongly converging beam through L4. At a representative semi-diameter of 12 mm, the aspherical departure is −0.158 mm from the base sphere.

### 3.5 Lens Unit L5 — Focus Group A (f = −47.42 mm)

**Element 20 — Negative meniscus, nd = 1.72916, νd = 54.7**
R₁ = +157.315, R₂ = +28.256 (both R positive; R₁ > R₂ → concave toward object). FL = −47.42 mm. S-LAL18 lanthanum crown (same glass as elements 3 and 4). This single-element unit is one of the two floating focus groups. It is driven by one of the dual Nano USM motors and moves **toward the image plane** when focusing from infinity to close distance (¶0069). Its compact size (1.20 mm center thickness) enables the fast, quiet autofocus performance required for video.
...
All negative polynomial coefficients: the surface flattens progressively toward the edge. This is the strongest aspherical correction in the design — at h = 12 mm, the departure reaches −0.158 mm. The glass is OHARA L-LAH85V (PGM-designated dense lanthanum flint).
...
**Precision glass molding (PGM) glasses (2 elements):** The two GMo elements use OHARA PGM-designated glasses: element 18 (L-LAH85V, nd = 1.854, νd = 40.4) and element 21 (L-BAL42, nd = 1.583, νd = 59.4). The "L-" prefix in OHARA's nomenclature specifically denotes glasses formulated for precision glass molding processes, with appropriate thermal characteristics for the moulding temperatures and pressures involved. These are distinct from OHARA's "S-" prefix conventional polished glasses.

**Dense lanthanum glasses (8 elements):** Elements 1 and 15 use TAFD37A; elements 3, 4, and 20 use S-LAL18; element 5 uses S-LAH58; element 13 uses S-LAH66; and element 18 uses L-LAH85V. The remaining high-index elements — element 8 (nd = 1.770, νd = 29.7, still code-only) and element 9 (S-TIH53) — belong to the heavy flint family and serve as chromatic correction partners in their respective units.
...
**PGM glass selection.** Both GMo elements use OHARA PGM-designated glasses (L-LAH85V and L-BAL42) specifically formulated for the glass moulding process, ensuring correct optical properties are maintained through the thermal forming cycle.

---

*Analysis generated from US 2024/0192474 A1, Example 4. All focal lengths, Petzval sums, and numerical claims independently verified via paraxial ABCD matrix ray trace. The 2026-05-19 audit promoted most six-digit family labels to public catalog matches; element 8's 770/297 heavy-flint row remains code-only because no coefficient-backed public catalog match was found. Production specifications sourced from Canon's official spec sheet and marketing materials.*
