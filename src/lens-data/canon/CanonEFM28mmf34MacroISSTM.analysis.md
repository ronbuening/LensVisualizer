...
The `.data.ts` file authors one pair of endpoint states: the first-mode infinity state and the third finite 1.2× state. The resulting variable gaps are `FC: 0.94 → 10.32 mm` and `d22: 29.28 → 20.49 mm` ahead of the plate, with the 12.28 mm final air fixed (air-equivalent BF `42.219 → 33.429 mm`). This preserves the patent's extreme optical states but should not be interpreted as a full simulation of the lens's two user-selectable focus modes. Both endpoints are selectable source states. The 1.2× source distance is **calculated**, 15.5451 mm before the first surface (92.5951 mm from the image plane), including the physical rear plate. Independent exact rays reproduce the published reproduction ratio within 0.093%. The other two published finite configurations are not authored keyframes and remain unavailable for selection.
