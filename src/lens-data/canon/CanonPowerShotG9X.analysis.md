...
The source's final plane-parallel optical block at surfaces 16–17 (a filter member such as a quartz low-pass or IR-cut filter, per ¶0070) is modeled in `rearPlates`: 1.09 mm at nd = 1.51633, νd = 64.1 (S-BSL7 class), followed by the published 1.62 mm spacing to the image plane. Every analysis traces through it, but it is not drawn. The last lens surface stores the patent's physical d15 gaps of 5.14, 3.38, and 2.74 mm. The block's air-equivalent 0.718840886 mm plus the 1.62 mm spacing reproduces the patent's air-equivalent BF of 7.48, 5.72, and 5.08 mm (7.478840886, 5.718840886, and 5.078840886 mm unrounded), so EFL and paraxial image position are unchanged. Physical track now includes the block and is 1.09 × (1 − 1/1.51633) = 0.371 mm longer than the air-equivalent track; the patent's printed lens total length (52.34 / 49.73 / 53.96 mm) uses the air-equivalent BF.
...
Apart from the patent's own filter block in `rearPlates`, no additional sensor cover glass, dummy surface, flare-cutter plane, synthetic cement layer, focus reconstruction, IS decenter, or geometric scale factor is introduced. The only source correction carried into the interpretation is the `νd2p`/`Nd2p` symbol contradiction in condition (9)/(9a).
