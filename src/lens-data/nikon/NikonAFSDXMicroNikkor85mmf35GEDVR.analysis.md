...
The final data structure stores all three published rows as exact focus keyframes. The β = -0.5 station is not reproduced by a single endpoint interpolation because G2 and G3 reach it at slightly different normalized fractions, so the explicit middle vector is required. Continuous motion between the source rows remains piecewise-linear visualization.
...
The binding inferred geometry occurs across the 1.300 mm air gap between surfaces 4 and 5. At the 13.0 mm shared material rim, the physical surface clearance remains +0.139453 mm under the default `gapSagFrac = 0.90` policy. The minimum computed element edge thickness is 1.098641 mm and the maximum spherical rim-slope angle is 28.987°. These quantities describe validation of the inferred clear apertures, not patent-specified dimensions.
