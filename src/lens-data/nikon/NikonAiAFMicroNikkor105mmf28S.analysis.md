...
The data representation requires one qualification. The current prime-lens `var` structure stores only infinity and close-focus endpoint pairs under a single linear focus parameter. It therefore reproduces the published infinity and β = −1.0 endpoints exactly but cannot encode the β = −0.5 keyframe exactly. The normalized parameter required by d6 is 0.599434, whereas d11 requires 0.521207, and d15 reverses 10.000 → 7.238 → 10.000 mm. Consequently, runtime interpolation between the two stored endpoints is only a visualization interpolation; it is not asserted to be the patent's physical focus trajectory. No intermediate state was reconstructed or invented.
