...
- Direct subfolders: 0
- Main outbound areas: same folder (23), src/generated (6), src/types (4), src/utils/seo (3), glob:../../lens-data/**/*.analysis.md, glob:../../lens-data/**/*.data.ts, src/comparison, src/lens-data/defaults.ts, +1 more
...
| `patentCatalog.ts` | Patent Catalog helper module | same folder (3) | src/pages/PatentsIndexPage.tsx | PatentLens, PatentJurisdiction, PatentRecord, PatentAssigneeGroup, PatentCountryGroup, PatentIndex, PATENT_ASSIGNEE_FALLBACK, espacenetPatentUrl, +4 more |
| `relationshipGraph.ts` | Relationship Graph helper module | same folder (3) | src/components/relationshipMap (4), src/pages/RelationshipMapPage.tsx | PartyRole, PartyRef, GraphPatentNode, GraphPartyNode, RelationshipGraph, resolveFocusParam, buildRelationshipGraph |
