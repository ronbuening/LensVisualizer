...
export function createInitialState(
  ...
  return {
    ...
    rays: {
      ...
      chromV: prefs.chromV ?? false,
      showPupils: prefs.showPupils ?? false,
      showCardinals: prefs.showCardinals ?? false,
      showCardinalFocal: prefs.showCardinalFocal ?? true,
      showCardinalPrincipal: prefs.showCardinalPrincipal ?? true,
      showCardinalNodal: prefs.showCardinalNodal ?? true,
      showCardinalDimensions: prefs.showCardinalDimensions ?? false,
      showCardinalEfl: prefs.showCardinalEfl ?? true,
      showCardinalBfd: prefs.showCardinalBfd ?? true,
      showCardinalFfd: prefs.showCardinalFfd ?? true,
      showCardinalHiatus: prefs.showCardinalHiatus ?? true,
      showCardinalTotalTrack: prefs.showCardinalTotalTrack ?? true,
    },
    sliders: {
      focusT: urlState.focus ?? 0,
      zoomT: 0,
      aberrationT: urlState.aberration ?? 0,
    ...
    panels: {
      ...
      chromaticOverlayOpen: urlState.chromaticOverlayOpen ?? false,
      petzvalOverlayOpen: urlState.petzvalOverlayOpen ?? false,
      showEffectiveFocalLength: prefs.showEffectiveFocalLength ?? false,
...
export default function lensReducer(state: LensState, action: LensAction): LensState {
  ...
  switch (action.type) {
    ...
    case APPLY_URL_VIEW_STATE: {
      ...
      if ("aberration" in urlState && !state.lens.comparing) {
        sliders.aberrationT = urlState.aberration ?? 0;
      }
