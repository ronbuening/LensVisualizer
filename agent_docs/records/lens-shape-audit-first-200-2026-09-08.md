...
Per-lens evidence lives in each lens's `*.audit.md` sidecar; this file only holds the queue and resume state.
...
- Batch 41–50 committed under “Audit hosted lens diagrams 41–50”. Locate later batches by title with
...
- A consolidated user-facing changelog entry for the resumed run is owed at its end; batches 41–50 onward have none
  yet.
...
Items from the completed batches that are still open beyond the per-lens limitations listed in the handoff. Each is
also recorded in the lens's own `*.audit.md`.

- Lens 13 (GR III): the printed rear filter order conflicts with its figure; the inferred repair is explicit. Lens 14
  (GR IV): two conflicting printed glass names are retained as a source conflict, numerical coordinates authoritative.
- Lens 15 (Z 70–200): the stated total length 199.88619 mm equals the first-to-last-glass span although the source
  definition includes BFD (the table sums to about 232.43 mm with BF); not rescaled.
- Lens 16 (Z 24–70) opts into `zoomApertureModel: "from-nominal-fno"` with exact-ray inferred station iris radii — an
  explicit reconstruction, not a patent-listed stop schedule. Lenses 36, 38 and 39 later took the same inferred
  schedule; lens 40 drives the stop from published `zoomStopSemiDiameters`. Other zooms keep their existing iris
  behaviour until their own source review in the queue.
- Lens 21 L4/G4 figure-versus-clearance mismatch, lens 22 front rim versus the source aperture's minimum pupil
  diameter, and lens 24 S12/S14 held at 15.8 mm to avoid hidden stop-adjacent trimming are each queued in their audit
  logs; conservative rims are retained.
