...
export function ambiguityGroupKey(
...
export function renderGlassAmbiguityScan(modules: GlassLensModules) {
  ...
  totalLenses = walkLensSurfaces(modules, ({ filePath, data, L: lens }) => {
    for (const element of lens.elements) {
      ...
      assert.equal(explanation.selected.name, runtimeSelection?.name);
  ...
  rows.sort(
    (a, b) => a.lensName.localeCompare(b.lensName) || a.filePath.localeCompare(b.filePath) || a.elementId - b.elementId,
  );
  ...
  emitReport(`${REPORT_DIR}/glass-ambiguities.generated.md`, lines.join("\n") + "\n");
